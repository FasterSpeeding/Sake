# Sake contributing guidelines

First let me thank you in advanced for any contributions you may make to Sake; contributions are always welcome
...
The most important thing to consider while contributing towards Sake is the checks the library's run against.
...
1. Ensure your current working directory is Sake's top-level directory.
...
* All modules present in Sake should start with the commented out licence (including the source encoding and cython
  language level declarations), a relevant component documentation string, `from __future__ import annotations`, an
 `__all__` declaration and then imports. For an example see any of Sake's current components.
...
* All top-level modules should be explicitly imported into `sake.__init__` and included in `sake.__init__.__all__`
* for type-completness with only the most important of their contents needing to be included in `sake.__init__.__all__`.
